...
(async function () {
    ...
    var styleSheet, editor, timer;

    // Global variables
    var unit, x, velocity, sensitivity, speedMultip, relativeLimit, steps, play, timeoutStatus, invertedWheel, focus, promptStyleOption, customStyle, flipV, flipH, fontSize, promptWidth, focusHeight, promptHeight, previousPromptHeight, screenHeight, previousScreenHeight, screenWidth, previousVerticalDisplacementCorrector, domain, debug, closing, cap, syncDelay;
    ...
    async function updateContents() {
        ...
        prompt.innerHTML = decodeURIComponent(session?.html || "Awaiting Data...");
        updateVelocity();
    ...
    function restoreRequest() {
        ...
        if (editor)
            editor.postMessage( {'request':command.restoreEditor} );
    ...
    function syncPrompters() {
        editor.postMessage( {'request':command.sync,'data':getProgress()} );
    }

    function instaSync() {
        if (steps>syncDelay)
            editor.postMessage( {'request':command.iSync,'data':getProgress()} );
    }
    ...
    function stopAll() {
        editor.postMessage( {'request':command.stopAll} );
    }
    ...
    function moveToAnchor( theAnchor ) {
        editor.postMessage( {'request':command.anchor,'data':theAnchor} );
    }
    ...
    function increaseFontSize() {
        editor.postMessage( {'request':11, 'data':getProgress()} );
    }
    function decreaseFontSize() {
        editor.postMessage( {'request':12, 'data':getProgress()} );
    }
    ...
    function decreaseVelocity() {
        editor.postMessage( {'request':2,'data':getProgress()} );
    }

    function increaseVelocity() {
        editor.postMessage( {'request':1,'data':getProgress()} );
    }
    ...
    function internalPauseAnimation() {
        editor.postMessage( {'request':command.internalPause} );
    }

    function internalPlayAnimation() {
        editor.postMessage( {'request':command.internalPlay} );
    }
